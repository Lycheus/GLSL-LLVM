  ...
  if(strcmp(p_tok,"OpenGLES")==0)
      ...
      else if(strncmp(temp_fun_name,"MatrixTimesVector",17)==0)
	{
	  ...
	  if(atoi(&temp_fun_name[17])==4)
	    ...
	    GL_MatrixTimesVector(RETU_REGI,ARGU_REGI+3,ARGU_REGI+2,ARGU_REGI+1,0,ARGU_REGI,ARGU_REGI+4,0,atoi(&temp_fun_name[17]),out);
	  else if(atoi(&temp_fun_name[17])==2)
	    GL_MatrixTimesVector(RETU_REGI,ARGU_REGI+2,ARGU_REGI+1,0,0,ARGU_REGI,ARGU_REGI+3,0,atoi(&temp_fun_name[17]),out);
	  else if(atoi(&temp_fun_name[17])==1)
	    GL_MatrixTimesVector(RETU_REGI,ARGU_REGI+1,0,0,0,ARGU_REGI,ARGU_REGI+2,0,atoi(&temp_fun_name[17]),out);
	  //GL_MatrixTimesVector(RETU_REGI,ARGU_REGI+4,ARGU_REGI+3,ARGU_REGI+2,ARGU_REGI+1,ARGU_REGI,ARGU_REGI+5,0,atoi(&temp_fun_name[17]),out);
	}
      ...
      else if(strncmp(temp_fun_name,"VaryingPut",10)==0)
	{
	  ...
	  for(i=1;i<max_number_of_varying_variables ;i++)
		...
		{
		  GL_VaryingPut(32+i,atoi(&temp_fun_name[10]),NULL);
		  break;
		}
	      if(v_form.index[i]==-1)
		{
...
void type_2(int input_class,int argu_number,int F)
{
  //printf("type_2: %s\targu_num: %d\tF:%d\n",decode_line[0],argu_number,F);  
...
void make_table(char *filename,char input_type,struct programobject *input_object)
{
  ...
  while(nEOF)
      ...
      if(!nEOF)
	{
      ...
      //printf("1. PC: %d\tdecode_line:%s %s %s\n",address,decode_line[0],decode_line[1],decode_line[2]);
...
void output_the_user_defining_function(char* filename,char type){
  ...
  while(nEOF){
    ...
    fflush(stdout);
    if(nowRead[0] != '.' && nowRead[0] != '\t' && nowRead[0] != '#'){
      strcpy(tempRead,nowRead);
      pch = strtok(nowRead," #");
      pch = strtok(NULL," #");  //Get the second token to check if the string is equal to "main". 
      if(pch!=NULL){
	if(strncmp(pch,"@main",5)){ //If nowRead != "main" then ok. It will cut user defining function down and then paste below the main function.
	  //printf("pch:%s\n",pch);
	  fputs(tempRead,fp1);
	  nEOF = fgets(nowRead,200,read_file);
	...
	else{ //put main function into main_x file.
	  fputs(tempRead,fp);
	  continue;
	}
      }
      else{     //puts variable name into main_x file. e.g. Position_temp:
	fputs(tempRead,fp);
	continue;
      }
    }
...
#endif
{
  ...
  int i,j;
  int endofmain=0;
  struct uniform_form temp_u_form;
  u_form=&temp_u_form;
  ...
  for(int index=0;index<1;index++){
    ...
    if(type == 'v')
      sprintf(tempRead,"mv main_v.txt %s",filename[0]);
    else
      sprintf(tempRead,"mv main_f.txt %s",filename[0]);
    system(tempRead);
  }
