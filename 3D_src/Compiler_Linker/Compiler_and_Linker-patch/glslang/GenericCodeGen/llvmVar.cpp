		   ...
		   const TllvmVarType lvt)
{
    ...
    name = n;
    vartype = lvt;
    DoesIChangeType =false;
    ExpandToVectors=false;
}
						...
						vartype(TllvmVarConstant)
{
    ...
    isb << ">";
    */
    char GetStringOfIEEE754[256];//Kaymon
    TransBinFloat( fvec[0] , GetStringOfIEEE754 );
    isb << "<float " << GetStringOfIEEE754;
    for(i=1;i<size;++i){
      TransBinFloat( fvec[i] , GetStringOfIEEE754 );
	isb << ", float " << GetStringOfIEEE754;
    }
    isb << ">";
    name = isb.c_str();
