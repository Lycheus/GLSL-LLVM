...
bool LLParser::ParseTopLevelEntities() {
  while (1) {
    ...
    case lltok::kw_protected: {   // OptionalVisibility
     ...
     case lltok::kw_uniform:        // OptionalLinkage
     case lltok::kw_varying:        // OptionalLinkage
     case lltok::kw_attribute:      // OptionalLinkage
     {
...
bool LLParser::ParseGlobalType(bool &IsConstant) {
  ...
  //dondum
  else if (Lex.getKind() == lltok::kw_uniform)  
    IsConstant = false;
  else if (Lex.getKind() == lltok::kw_varying)
    IsConstant = false;
  else if (Lex.getKind() == lltok::kw_attribute)
    IsConstant = false;
    ...
    IsConstant = false;
    //dondum
    return TokError("expected 'global' or 'constant' or 'uniform', 'attribute', 'varying'");
  }
  Lex.Lex();
  return false;
}
...
bool LLParser::ParseNamedGlobal() {
  ...
  //dondum
  if( (Lex.getKind()==lltok::kw_uniform) ||  (Lex.getKind()==lltok::kw_attribute) ||  (Lex.getKind()==lltok::kw_varying) )
     ParseOptionalLinkage(Linkage,HasLinkage);
...
bool LLParser::ParseOptionalLinkage(unsigned &Res, bool &HasLinkage) {
  ...
  case lltok::kw_external:       Res = GlobalValue::ExternalLinkage;      break;
    ...
    //dondum 
    /* Dondum 2009-11-02. 2009-11-10 here has some question . something to do here?  2009-11-13. yes! it can't Lex.Lex(), or global type cannot be OK */
  case lltok::kw_highp:          Res = GlobalValue::HighpLinkage;         break;
  case lltok::kw_attribute:      Res = GlobalValue::AttributeLinkage;     { HasLinkage=true; return false;} break;
  case lltok::kw_uniform:        Res = GlobalValue::UniformLinkage;       { HasLinkage=true; return false;}  break;//Dondum 2009-11-10
  case lltok::kw_varying:        Res = GlobalValue::VaryingLinkage;       { HasLinkage=true; return false;}  break;
...
bool LLParser::ParseParameterList(SmallVectorImpl<ParamInfo> &ArgList,
  ...
  while (Lex.getKind() != lltok::rparen) {
    ...
    // Dondum 2009-11-14 
    // let highp do nothing and pass the "highp < 4 x float>" form.
    if (Lex.getKind() == lltok::kw_highp) Lex.Lex();
...
bool LLParser::ParseArrayVectorType(PATypeHolder &Result, bool isVector) {
 ...
 // /* add: Dondum 2009-12-24, < 4 x 4 float > by Godfrey
   LocTy MtxSizeLoc; // Goda 2009-12-02
   uint64_t MtxSize; // Goda 2009-12-02
   int  ItIsReallyAMatrix; // Goda 2009-12-09
   if(Lex.getKind() != lltok::APSInt || Lex.getAPSIntVal().isSigned() || // Goda 2009-11-18
      Lex.getAPSIntVal().getBitWidth() > 64) { // Goda 2009-11-16
     //    printf(" ItIsReallyAMatrix=0\n");
     ItIsReallyAMatrix=0;
   }
   else { // Goda 2009-11-16
     ItIsReallyAMatrix = 1; // Goda 2009-12-09
     MtxSizeLoc = Lex.getLoc(); // Goda 2009-12-02 
     MtxSize = Lex.getAPSIntVal().getZExtValue(); // Goda 2009-12-02
     Lex.Lex(); // Goda 2009-11-16 
   }
   //  */ 
   LocTy TypeLoc = Lex.getLoc();
   PATypeHolder EltTy(Type::getVoidTy(Context));
   // /* if (ParseTypeRec(EltTy)) return true; 
   if (ParseTypeRec(EltTy)) { // Goda 2009-12-02  
	 if (Lex.getKind() == lltok::Type) return true; // Goda 2009-12-02 
	 else // Goda 2009-12-10   
     {
       if (!ItIsReallyAMatrix && Lex.getKind() == lltok::APSInt) // Goda 2009-12-10  
	 return TokError("expected number in address space"); // Goda 2009-12-02 
	 }
   }
   ...
   if (isVector) {
     ...
     if(ItIsReallyAMatrix)
       {
	 if(MtxSize!=Size)
	   return Error(MtxSizeLoc, "not a square matrix");
	 // It's MatrixType. Dondum 2010-01-27 
	 Result = VectorType::get(EltTy, Size);
	 //	 printf("Matrix size all add 5 ))) %d\n",Size);
	 //dondum testing
	 //	 printf("Result = %d \n");
       } else {
...
bool LLParser::ParseFunctionHeader(Function *&Fn, bool isDefine) {
  ...
  //2009-11-03 Dondum.Let highp always OK
  case GlobalValue::HighpLinkage:
    break; // always ok.
  ...
  case GlobalValue::CommonLinkage:
    ...
    //dondum
  case GlobalValue::AttributeLinkage:
  case GlobalValue::UniformLinkage:
  case GlobalValue::VaryingLinkage:
...
bool LLParser::ParseCall(Instruction *&Inst, PerFunctionState &PFS,
  ...
  //Dondum 2009-11-14. For pass "call highp", let highp do nothing.
  if (Lex.getKind() == lltok::kw_highp) Lex.Lex();
