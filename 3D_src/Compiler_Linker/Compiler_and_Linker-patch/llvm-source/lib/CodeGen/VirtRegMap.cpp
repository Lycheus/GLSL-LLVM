...
//added by sheng-chih
cl::opt<bool> UseGPUStackAddress("use-gpu-stack-address",
				 cl::init(false),
...
//fixed by sheng-chih
  ...
  if(UseGPUStackAddress)
    {
      int Size = RC->getSize()/16;
      int Align = RC->getAlignment()/16;
      if(Size == 0)
	Size = 1;
      if(Align == 0)
	Align = 1;
      SS = MF->getFrameInfo()->CreateSpillStackObject(Size,
						      Align);
    }
  else
    SS = MF->getFrameInfo()->CreateSpillStackObject(RC->getSize(),
						    RC->getAlignment());
//end fixed
