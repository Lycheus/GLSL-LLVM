...
private:
  ...
  SDValue EmitStackConvert2(SDValue SrcOp, EVT SlotVT, EVT DestVT, DebugLoc dl);//EXCESS4
...
SDValue SelectionDAGLegalize::LegalizeOp(SDValue Op) {
  ...
  //printf("the code will come here?\n");//EXCESS4
    ...
    if (ExtType == ISD::NON_EXTLOAD) {
      ...
      case TargetLowering::Legal:
        ...
        if (!TLI.allowsUnalignedMemoryAccesses(LD->getMemoryVT())) {
          ...
          if (LD->getAlignment() < ABIAlignment){                   //Kaymon If the LD alignment (might be from the LLVM IR alignment )differ from the calculating alignment. Generate the redundant DAG for load.
...
SDValue SelectionDAGLegalize::EmitStackConvert2(SDValue SrcOp,
                                               EVT SlotVT,
                                               EVT DestVT,
                                               DebugLoc dl) {
  // Create the stack frame object.
  ...
  //printf("%d\n%d\n%d\n%d\n%d\n",SrcSize,SlotSize,DestSize,DestAlign,SrcAlign);//EXCESS4
  ...
  {
    //printf("enter this Load\n");
    
    SDValue Load =  DAG.getLoad(DestVT, dl, Store, FIPtr, SV, 0, false, false,
                       DestAlign);
    SDValue Result = DAG.getNode(ISD::FABS, dl, DestVT, Load,DAG.getIntPtrConstant(0)); //EXCESS4
...
SDValue SelectionDAGLegalize::ExpandLegalINT_TO_FP(bool isSigned,
  ...
  if (Op0.getValueType() == MVT::i32) {
    ...
    //SDValue Sub = DAG.getNode(ISD::FADD, dl, MVT::f64, Load, Bias);
    //SDValue Sub = DAG.getNode(ISD::FADD, dl, MVT::f64, Load, Bias);
    // EXCESS4
    // final result
    SDValue Result;
    ...
    if (DestVT == MVT::f64) {
      // do nothing
      //printf("   came into block 5\n");//EXCESS4
      Result = Sub;
    } else if (DestVT.bitsLT(MVT::f64)) {
      //printf("   came into block 6\n");
      Result = DAG.getNode(ISD::FP_ROUND, dl, DestVT, Sub,
                           DAG.getIntPtrConstant(0));
      //EXCESS4 did terrible thing here
      //Result = Sub
    } else if (DestVT.bitsGT(MVT::f64)) {
      //printf("   came into block 7\n");
      Result = DAG.getNode(ISD::FP_EXTEND, dl, DestVT, Sub);
    }
    //printf("leaving\n");
    return Result;
  }
  //printf(" came into block 8\n"); //EXCESS4
...
SDValue SelectionDAGLegalize::PromoteLegalFP_TO_INT(SDValue LegalOp,
                                                    ...
                                                    bool isSigned,
                                                    DebugLoc dl) {
  //printf("show me the message if come FP TO SINT\n");//EXCESS4
...
void SelectionDAGLegalize::ExpandNode(SDNode *Node,
  ...
  case ISD::FP_ROUND_INREG: {
    ...
    //printf("come into FP_ROUND_INREG\n");
    //EXCESS4
    EVT ExtraVT = cast<VTSDNode>(Node->getOperand(1))->getVT();
  ...
  case ISD::SINT_TO_FP:
  case ISD::UINT_TO_FP:
     //EXCESS4 did a terrible wrok here
    Tmp1 = EmitStackConvert2(Node->getOperand(0),Node->getOperand(0).getValueType(),Node->getValueType(0), dl);
    //Tmp1 = ExpandLegalINT_TO_FP(Node->getOpcode() == ISD::SINT_TO_FP,
    //                             Node->getOperand(0), Node->getValueType(0), dl);
    Results.push_back(Tmp1);
    break;
  case ISD::FP_TO_UINT: {
    //printf("come into FP_TO_UINT\n");
    //EXCESS4
    SDValue True, False;
