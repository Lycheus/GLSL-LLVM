...
static void getCopyToPartsVector(SelectionDAG &DAG, DebugLoc DL,
  ...
  if (NumParts == 1) {
    if (PartVT == ValueVT) {
      ...
      //yakola - For sloving extractelt inst problem
      // assert(ValueVT.getVectorElementType() == PartVT &&
      //     ValueVT.getVectorNumElements() == 1 &&
...
// yakola - for solving extractelement problem
void SelectionDAGBuilder::visitExtElt(User &I) {
  SDValue InVec = getValue(I.getOperand(0));
  SDValue InIdx = DAG.getNode(ISD::ZERO_EXTEND, getCurDebugLoc(),
                                TLI.getPointerTy(),
                                getValue(I.getOperand(1)));
...
cl::opt<bool> UseNewMatrixAddress("use-new-matrix-address",
                                  cl::init(false),
                                  cl::desc("Use GPU-new-matrix address"),
                                  cl::Hidden); //Kaymon add this option to change the matrix address. Since the address mode is 16 bytes, we divide any imm by 16.
void SelectionDAGBuilder::visitGetElementPtr(const User &I) {
  SDValue N = getValue(I.getOperand(0));
  const Type *Ty = I.getOperand(0)->getType();
  const Type *Ty2 = I.getOperand(0)->getType();
  //cout<<"visitGetElementPtr\n";
  for (GetElementPtrInst::const_op_iterator OI = I.op_begin()+1, E = I.op_end();
	...
	if(Offset % 16 == 0)//Temporary  solution for structure. Kaymon
	  Offset /= 16;
	else if (Offset %4 == 0){
	  Offset /=4;
	  structure_flag=1;
	...
	if(UseNewMatrixAddress){//Kaymon add this option to change the matrix address. Since the address mode is 16 bytes, we divide any imm by 16.
	  /*
	  if(Offs % 16 == 0)
	    Offs /= 16;
	  else if (Offs %4 == 0) //For array address. e.g. int a[10];
	    Offs /=4;
	  */
	  /*
	  cout<< Ty->isArrayTy() <<"       array =======1\n";
	  cout<< Ty->isVectorTy()<<"       vector 2\n";
	  cout<< Ty->isAggregateType()<<"     aggregate   3\n";
	  cout<< Ty->isIntegerTy()<<"     int   4\n";
	  cout<< Ty->isFloatingPointTy()<<"    ieee754   5\n";
	  cout<< Ty->isIntOrIntVectorTy()<<"    int or i32   6\n";
	  cout<< N.getOpcode()<<"       7.opcode\n";
	  cout<< N.getMachineOpcode()   <<" =========machineopeocde\n";
	  
	  cout<< Offs <<" offs\n";
	  */
	  if(Ty->isVectorTy()||Ty->isAggregateType()){//For vector array. e.g. mat3 a => vec3 a[3].
	    Offs /= 16;
	    //cout<<Offs<<" "<<Offs*4<<"I am here v\n";
	  }
	  else if (Ty->isFloatingPointTy()){ //For floating point array address. e.g. int a[10];
	    Offs /=4;
	    FloatVariable=1;
	    //cout<<Offs<<" "<<Offs*4<<"I am here f\n";
	  }
	  /*
	  if(count==1){
	    cout<<"count: "<<count-1<<"\n";
	...
	//cout<< "===========end\n";
      ...
      //Fixed by Sheng-chih
      //Fix getElementPtr to get our hardware address
      //Our hardware's memory unit is 16 bytes;
      //  ElementSize = ElementSize / 16;
      //  if(ElementSize % 16)
      //  ElementSize++;
      if(UseGPUStackAddress)
	{
	  long temp_long = ElementSize.getSExtValue();
	  temp_long /=16;
	  if(ElementSize.getSExtValue()%16)
	    temp_long++;
	  ElementSize=APInt(TLI.getPointerTy().getSizeInBits(),temp_long);
	}
      //end fixed
