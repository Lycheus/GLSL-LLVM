...
void ScheduleDAGSDNodes::AddSchedEdges() {
  ...
  for (unsigned su = 0, e = SUnits.size(); su != e; ++su) {
    ...
    //cout<<"getOpcode."<<su<< ": " <<MainNode->getMachineOpcode() << " "<<MainNode->getOperationName()<<"\n"; Kaymon
    fflush(stdout);
...
MachineBasicBlock *ScheduleDAGSDNodes::EmitSchedule() {
  ...
  for (unsigned i = 0, e = Sequence.size(); i != e; i++) {
    ...
    //cout<< i<<": Scheduled instrs: " << SU->getNode()->getMachineOpcode()<<" "<<SU->getNode()->getOperationName()<<"\n"; //Kaymon
    /*
    if(SU->getNode()->getMachineOpcode()==228){
      if(Sequence[i+1]->getNode()->getMachineOpcode()==224&&Sequence[i+2]->getNode()->getMachineOpcode()==224&&Sequence[i+3]->getNode()->getMachineOpcode()==224)
	if(Sequence[i+4]->getNode()->getMachineOpcode()==327&&Sequence[i+5]->getNode()->getMachineOpcode()==327&&Sequence[i+6]->getNode()->getMachineOpcode()==327&&Sequence[i+7]->getNode()->getMachineOpcode()==327)
	  temp_flag=8;
    }
    if(temp_flag==8||temp_flag==4||temp_flag<=0){
      temp_flag--;
    }
    else{
      temp_flag--;
