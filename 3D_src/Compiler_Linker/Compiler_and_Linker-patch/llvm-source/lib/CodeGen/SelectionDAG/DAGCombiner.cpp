...
void DAGCombiner::Run(CombineLevel AtLevel) {
  ...
  while (!WorkList.empty()) {
    ...
    //sheng-chang
    bool debug = false;
    std::string str = N->getOperationName();
    if (debug){
      errs()<< N->getOperationName() <<"\n";
      if(Level >= NoIllegalOperations)
        if(str.compare("extract_vector_elt")==0)
          DAG.viewGraph("before combine extract_element");
    }
    ...
    if (debug){
      if(Level >= NoIllegalOperations)
        if(str.compare("extract_vector_elt")==0)
          DAG.viewGraph("after combine extract_element");
    }
...
SDValue DAGCombiner::visitEXTRACT_VECTOR_ELT(SDNode *N) {
 ...
 if (InVec.getOpcode() == ISD::SCALAR_TO_VECTOR) {
  ...
  //sheng-chang
  bool debug = false;
  if(Level >= NoIllegalOperations && debug)
    DAG.viewGraph("visit extract_vector_element1");
  ...
  if (isa<ConstantSDNode>(EltNo)) {
    ...
    //sheng-chang
    if(Level >= NoIllegalOperations && debug)
      DAG.viewGraph("visit extract_vector_element2");
    ...
    //sheng-chang
    if(Level >= NoIllegalOperations && debug)
      DAG.viewGraph("visit extract_vector_element3");
    ...
    //yakola
    if(Level >= NoIllegalOperations && debug)
      DAG.viewGraph("visit extract_vector_element4");
    if(Level >= NoIllegalOperations)
      return SDValue();
    ...
    //sheng-chang
    if(Level >= NoIllegalOperations && debug)
      DAG.viewGraph("visit extract_vector_element5");
