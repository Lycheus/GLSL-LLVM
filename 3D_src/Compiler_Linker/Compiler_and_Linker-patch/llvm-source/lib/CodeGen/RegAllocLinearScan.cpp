...
namespace {
  ...
  //static cl::opt<unsigned>  //different start
  static cl::opt<unsigned> NumRecentlyUsedRegs("linearscan-skip-count",
  ...
  struct RALinScan : public MachineFunctionPass {
    static char ID;          //different start
  ...
  private:
    ...
    BitVector reservedRegs_; //different
    LiveIntervals* li_;
    LiveStacks* ls_;
    MachineLoopInfo *loopInfo; //different
    ...
    SmallVector<unsigned, 4> RecentRegs;   //different start
  ...
  public:
    ...
    virtual void getAnalysisUsage(AnalysisUsage &AU) const {
      ...
      AU.addPreserved<SlotIndexes>(); //different
      if (StrongPHIElim)
        AU.addRequiredID(StrongPHIEliminationID);
      // Make sure PassManager knows which analyses to make available
      // to coalescing and which analyses coalescing invalidates.
      AU.addRequiredTransitive<RegisterCoalescer>();
      AU.addRequired<CalculateSpillWeights>(); //different
    ...
    bool isRecentlyUsed(unsigned reg) const {                      //different start
  ...
  private:
    ...
    unsigned getFirstNonReservedPhysReg(const TargetRegisterClass *RC) { //different start
    ...
    template <typename ItTy> //different
...
bool RALinScan::runOnMachineFunction(MachineFunction &fn) {
  ...
  //cout<<"before initintervalsets\n";
  //cout.flush();
  initIntervalSets();
  //dondum
  //cout.flush();
  //cout<<"Dump Machine function 1\n";
  //cout.flush();
  //fn.dump();
  //cout.flush();
  //cout<<"before linear scan\n";
  //cout.flush();
  linearScan();

  //cout<<"before rewriter\n";
  //cout.flush();
  // Rewrite spill code and update the PhysRegsUsed set.
  rewriter_->runOnMachineFunction(*mf_, *vrm_, li_);

  assert(unhandled_.empty() && "Unhandled live intervals remain!");
  //cout<<"before finalizereguses\n";
  //cout.flush();
  finalizeRegUses();
...
void RALinScan::linearScan() {
  ...
  while (!unhandled_.empty()) {
    ...
    //dondum try to do this.
    /*    const TargetRegisterClass *RC = mri_->getRegClass(cur->reg);
    if(RC->getID()==8 && !Flag_2 )
    {
      //printf("show me the (if)RC->getID() :%d\n",RC->getID());
      RE=RC;
    }
    else  if(RC->getID()==9 )
    {
      //printf("show me the (else)RC->getID() :%d\n",RC->getID());
    RE = RC;
    Flag_2 = 1;
    ...
    */
    ...
    {
      const TargetRegisterClass *RC = mri_->getRegClass(cur->reg);
      //            //printf("show me the RC now in AOSA:%d\n",RC);
      //    //printf("show me the RC->getID() :%d\n",RC->getID());
...
/// weight is the lowest among all the registers whose live intervals
/// conflict with the interval.
//EXCESS4_ From here the function decide the spill is empty or not
void RALinScan::findIntervalsToSpill(LiveInterval *cur,
  ...
  for (IntervalPtrs::iterator i = active_.begin(); i != active_.end(); ++i) {
    ...
    //    //printf("1 show me reg: %d and PhysReg: %d\n",Reg,PhysReg);//EXCESS4
    ...
    for (unsigned j = 0; j < NumCands; ++j) {
      ...
      //      //printf("show me all Candidate %d\n",Candidate);
  ...
  ////printf("BestCandidate: %d\n",BestCandidate); //EXCESS4
  ////printf("1.spill? : %d\n",SpillIntervals.empty());
  ////printf("SLIs[BestCandidate].begin(): %d ,SLIs[BestCandidate].end(): %d \n",SLIs[BestCandidate].begin(),SLIs[BestCandidate].end());
  
  std::copy(SLIs[BestCandidate].begin(), SLIs[BestCandidate].end(),
            std::back_inserter(SpillIntervals));
  ////printf("2.spill? : %d\n",SpillIntervals.empty()); //EXCESS4
}
...
void RALinScan::assignRegOrStackSlotAtInterval(LiveInterval* cur) {
  ...
  //const TargetRegisterClass *RC = mri_->getRegClass(cur->reg);
  //EXCESS4
  //RE=RC; it doesn't do the RE=RC,because a condition in linearScan will do it. 
  /* //Kaymon if segmentation fault occurs, enable this comment.
  if(!Flag_2){
  ...
  */
  if (cur->empty()) {
    ...
    if (!physReg)
      { 
	//dondum
	printf("physreg befoee getfirst:%d\n",physReg);
	physReg = getFirstNonReservedPhysReg(RE); 
	printf("after getfirst physreg:%d\n",physReg);
	// physReg = *RE->allocation_order_begin(*mf_);//EXCESS4
      }
    fflush(stdout);
  ...
  const TargetRegisterClass *RCLeader = RelatedRegClasses.getLeaderValue(RE);//EXCESS4
    ...
    if ((vni->def != SlotIndex()) && !vni->isUnused() &&
      ...
      if (CopyMI && CopyMI->isCopy()) {
        ...
        if (Reg) {
          ...
          if (DstSubReg)
            Reg = tri_->getMatchingSuperReg(Reg, DstSubReg, RE);
          if (Reg && allocatableRegs_[Reg] && RE->contains(Reg))
  ...
  //  //printf("gets to line 1043\n");
  //  //printf("Tell me if the register is not enough\n");
  //EXCESS4
  ...
  for (std::vector<std::pair<unsigned, float> >::iterator
    ...
    updateSpillWeights(SpillWeights, I->first, I->second, RE);
  ...
  for (IntervalPtrs::const_iterator i = active_.begin(), e = active_.end();
    ...
    assert(TargetRegisterInfo::isVirtualRegister(reg) &&
           "Can only allocate virtual registers!");
    reg = vrm_->getPhys(reg);
    updateSpillWeights(SpillWeights, reg, i->first->weight, RE);
  }
  ...
  if (!minReg || SpillWeights[minReg] == HUGE_VALF)
    for (TargetRegisterClass::iterator i = RE->allocation_order_begin(*mf_),
           e = RE->allocation_order_end(*mf_); i != e; ++i) {
      unsigned reg = *i;
  ...
  if (minWeight == HUGE_VALF) {
    ...
    //minReg = BestPhysReg ? BestPhysReg : *RE->allocation_order_begin(*mf_); //excess4
    minReg = BestPhysReg ? BestPhysReg : getFirstNonReservedPhysReg(RE); //dondum
	...
	//	//printf("It is spot 2\n");//EXCESS4
      ...
      //      //printf("gets to line 1123 return\n");
      return;
    }
  }
  
  //if(!spillIs.empty())
  //  //printf("why could not trace it?\n");
  //  //printf("gets to line 1129\n");
  ////printf("gets to line 1123  [%d]\n",spillIs.empty());
  ////printf(" TEST POINT \n");
  // EXCESS4
  ...
  //  //printf("gets to line 1132 and //printf RegsWeights.size() %d\n",LastCandidate);//EXCESS4
  ...
  if (cur->weight != HUGE_VALF && cur->weight <= minWeight) {
    ...
    spiller_->spill(cur, added, spillIs); //2.8 part~
    
    //    //printf("gets to line 1151\n"); //EXCESS4  mark
    ...
    //    //printf("gets to line 1190\n");
    return;
  }
  
  ++NumBacktracks;

  ++Myowncounter;//EXCESS4
  ...
  //  //printf("haven't choose any Register\n ");
  //EXCESS4
  ...
  assert(!spillIs.empty() && "No spill intervals?");//different
  SlotIndex earliestStart = spillIs[0]->beginIndex();//different
  ...
  //  //printf("Is spill empty?[%d]\n",spillIs.empty());//EXCESS4

  while (!spillIs.empty()) {
    // //printf("spill is empty [%d]",spillIs.empty());
    ////printf("It says that spill is not empty.\n") EXCESS4
    ...
    spiller_->spill(sli, added, spillIs);          //different
    addStackInterval(sli, ls_, li_, mri_, *vrm_); 
    //EXCESS4 
    // std::copy(added.begin(), added.end(), std::back_inserter(added));   
  ...
  while (!handled_.empty()) {
    ...
    //    //printf("show me the message if come here\n");//EXCESS4
...
//EXCESS4
/*
LiveInterval *fakecur;
int Flag =0;

const TargetRegisterClass *RD;
...
unsigned RALinScan::getFreePhysReg(LiveInterval *cur) {
  ...
  const TargetRegisterClass *RC = mri_->getRegClass(cur->reg);
  //printf(".header.RC->getID(): %d\n",RC->getID());
  if(RC->getID() == 8)
    //  if (!Flag) //excess4
  {
    //    const TargetRegisterClass *RC = mri_->getRegClass(cur->reg);
    //printf("1.if.RC->getID(): %d\n",RC->getID());
    RD=RC;
    Flag=1;

    const TargetRegisterClass *RCLeader = RelatedRegClasses.getLeaderValue(RC);
      //printf("1.if.RCLeader->getID(): %d\n",RCLeader->getID());
  ...
  // available first.
	...
	//      //printf("I want to see the Preference: %d",Preference);
      //EXCESS4
      return Preference;
      }
    }
    ...
    {
       unsigned FreeReg = getFreePhysReg(cur, RC, MaxInactiveCount, inactiveCounts,true);
       if (FreeReg)
       {
		//printf("\nI want to see FreeReg:%d\n",FreeReg);
	//EXCESS4
	  return FreeReg;
       }
    //printf("\nI want to see FreeReg:%d\n",FreeReg);
    ...
    //this if() do all the thing again
        //printf(" RD:%d RC:%d ",RD,RC); 
	// //printf(" Physics register : %d\n",getFreePhysReg(cur, RC, MaxInactiveCount, inactiveCounts, false));
    //EXCESS4
  ...
  else
    {
      ...
      //const TargetRegisterClass *RC = RD;
      //      RC = RD;
      //printf("2.else.RD->getID(): %d\n",RD->getID());
      //printf("2.else.RC->getID(): %d\n",RC->getID());
      //dondum marked.
      const TargetRegisterClass *RCLeader = RelatedRegClasses.getLeaderValue(RC);
      //printf("2.else.RCLeader->getID(): %d\n",RCLeader->getID());
...
unsigned RALinScan::getFreePhysReg(LiveInterval *cur) {
  ...
  //  //printf("QooQooQooQ\n");
    ...
    const TargetRegisterClass *RCLeader = RelatedRegClasses.getLeaderValue(RD);
    ...
    if (Preference) {
      DEBUG(dbgs() << "(preferred: " << tri_->getName(Preference) << ") ");
      if (isRegAvail(Preference) &&
	  RD->contains(Preference))
	return Preference;
    }

    if (!DowngradedRegs.empty()) {
      unsigned FreeReg = getFreePhysReg(cur, RD, MaxInactiveCount, inactiveCounts,
					true);
      if (FreeReg)
	return FreeReg;
    }
    return getFreePhysReg(cur, RD, MaxInactiveCount, inactiveCounts, false);
  }
  ...
  else
    {
      ...
      const TargetRegisterClass *RC;
      RC = RD;
      //      //printf("else part, RC->getID() must be %d.\n",RC->getID());
      const TargetRegisterClass *RCLeader = RelatedRegClasses.getLeaderValue(RC);
	...
	if (isRegAvail(Preference) &&
      ...
      //printf("!DowngradesRegs.empty(): %d \n",!DowngradedRegs.empty());
      if (!DowngradedRegs.empty()) {
	unsigned FreeReg = getFreePhysReg(cur, RC, MaxInactiveCount, inactiveCounts,true);
	...
	if (FreeReg)
      ...
      unsigned TestReg = getFreePhysReg(cur, RC, MaxInactiveCount, inactiveCounts, false);
