...
//added by sheng-chih
//ChangeToGPUMask
void MachineOperand::ChangeToGPUMask(char* GPUMask)
{
  // If this operand is currently a register operand, and if this is in a
  // function, deregister the operand from the register's use/def list.
  if (isReg() && getParent() && getParent()->getParent() &&
...
void MachineOperand::print(raw_ostream &OS, const TargetMachine *TM) const {
  ...
  case MachineOperand::MO_Register:
    ...
    break;
  case MachineOperand::MO_Immediate:
    if(flag_to_lwz == 1){ //For New::LWZ instructions. Kaymon Add a more operand to save the component information. This is for debug message.
      flag_to_lwz =0; 
      (getImm() == 0) ?OS <<"extract X component " :(getImm() == 4) ?OS <<"extract Y component " :(getImm() == 12) ?OS <<"extract W component " :(getImm() == 8) ?OS <<"extract Z component ":OS<<" " ;
    }
    else
      OS << getImm();
    break;
...
//added by sheng-chih
  case MachineOperand::MO_GPUMask:
    OS<< "Mask: "<<getGPUMask();
    break;
//end added
...
void MachineInstr::print(raw_ostream &OS, const TargetMachine *TM) const {
  ...
  for (unsigned i = StartOp, e = getNumOperands(); i != e; ++i) {
    ...
    if(getDesc().getName() == "LWZ"&& i== 3) //Kaymon add it to idetify the operand I add.
      flag_to_lwz = 1;
...
unsigned MachineInstr::Check_array_for_LFS_instruction() {
  int ReturnedValue = 0;
  for ( mmo_iterator i = MemRefs, e = MemRefsEnd; i != e; ++i) {                                                                                                        
    if((**i).getValue()==0)                                                                                                                                                                                
      ReturnedValue = 1;//Kaymon. if string is "mem: [<unknown>]", it means this LFS instructions is not belong with array.
    if (llvm::next(i) != e)                                                                                                                          
      cout << " ";                                                                                                                                                                                       
  }                                                                                                                                                                                                      
