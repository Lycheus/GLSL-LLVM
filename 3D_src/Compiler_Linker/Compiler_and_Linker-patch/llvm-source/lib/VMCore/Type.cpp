...
//yakola
VectorType::VectorType(const Type *ElType, unsigned NumEl, bool IsMatrix)
  : SequentialType(VectorTyID, ElType) {
  NumElements = NumEl;
  isMatrix = IsMatrix;
...
//yakola
VectorType *VectorType::getMatrix(const Type *ElementType,
                                  unsigned NumElements, bool isMatrix) {
  assert(ElementType && "Can't get vector of <null> types!");
  ...
  if (!PT) {
    pImpl->VectorTypes.add(PVT, PT = new VectorType(ElementType,
                                                    NumElements, isMatrix));
  }
#ifdef DEBUG_MERGE_TYPES
