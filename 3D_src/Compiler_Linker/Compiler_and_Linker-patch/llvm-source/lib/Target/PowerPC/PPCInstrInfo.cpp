...
void PPCInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  //  printf("tttttDst %d\n",DestReg);
  //printf("tttttSrc %d\n",SrcReg);
  //printf("GPRC: %d\n",PPC::GPRCRegClass.contains(DestReg, DestReg));
  //printf("VRRC: %d\n",PPC::VRRCRegClass.contains(SrcReg, SrcReg));
  unsigned Opc;
  //dondum
  int flag=0;
  ...
  if (PPC::GPRCRegClass.contains(DestReg, SrcReg))
    Opc = PPC::OR;
  //dondum 2010-11-07
  else if (PPC::GPRCRegClass.contains(DestReg, DestReg)&& PPC::VRRCRegClass.contains(SrcReg, SrcReg))
    {
      BuildMI(MBB, I, DL, get(PPC::VOR), DestReg).addReg(SrcReg).addReg(SrcReg, getKillRegState(KillSrc));
      flag=1;
    }
  else if (PPC::GPRCRegClass.contains(SrcReg, SrcReg)&& PPC::VRRCRegClass.contains(DestReg, DestReg))
    {
      BuildMI(MBB, I, DL, get(PPC::VOR), DestReg).addReg(SrcReg).addReg(SrcReg, getKillRegState(KillSrc));
      flag=1;
    }
