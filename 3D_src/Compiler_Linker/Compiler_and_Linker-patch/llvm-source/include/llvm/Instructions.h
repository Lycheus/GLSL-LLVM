...
// yakola - new instruciotn for solving extractelement instruction

class FakeExtractEltInst : public Instruction {
  FakeExtractEltInst(Value *Vec, Value *Idx, const Twine &NameStr = "",
                     Instruction *InsertBefore = 0);
  FakeExtractEltInst(Value *Vec, Value *Idx, const Twine &NameStr,
                     BasicBlock *InsertAtEnd);
protected:
  virtual FakeExtractEltInst *clone_impl() const;

public:
  static FakeExtractEltInst *Create(Value *Vec, Value *Idx,
                                   const Twine &NameStr = "",
                                   Instruction *InsertBefore = 0) {
    return new(2) FakeExtractEltInst(Vec, Idx, NameStr, InsertBefore);
  }
  static FakeExtractEltInst *Create(Value *Vec, Value *Idx,
                                   const Twine &NameStr,
                                   BasicBlock *InsertAtEnd) {
    return new(2) FakeExtractEltInst(Vec, Idx, NameStr, InsertAtEnd);
  }
  ...
  // Methods for support type inquiry through isa, cast, and dyn_cast:
  static inline bool classof(const FakeExtractEltInst *) { return true; }
  static inline bool classof(const Instruction *I) {
    return I->getOpcode() == Instruction::ExtElt;
  }
  static inline bool classof(const Value *V) {
    return isa<Instruction>(V) && classof(cast<Instruction>(V));
  }
...
template <>
struct OperandTraits<FakeExtractEltInst> : public FixedNumOperandTraits<2> {
};

DEFINE_TRANSPARENT_OPERAND_ACCESSORS(FakeExtractEltInst, Value)
...
public:
  ...
  //  virtual FakeExtractEltInst *clone() const;
