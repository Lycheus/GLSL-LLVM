...
//added by sheng-chih
  bool isGPUMask() const { return OpKind == MO_GPUMask; }
//end added
...
//added by sheng-chih
  ...
  const char* getGPUMask() const {
    assert(isGPUMask() && "Wrong MachineOperand accessor");
    return Contents.GPUMask;
  }

  void setGPUMask(char* GPUMask) {
    assert(isGPUMask() && "Wrong MachineOperand accessor");
    Contents.GPUMask[0] = GPUMask[0];
    Contents.GPUMask[1] = GPUMask[1];
    Contents.GPUMask[2] = GPUMask[2];
    Contents.GPUMask[3] = GPUMask[3];
    Contents.GPUMask[4] = GPUMask[4];
  }

  void ChangeToGPUMask(char* GPUMask);

  static MachineOperand CreateGPUMask(char* GPUMask) {
    MachineOperand Op(MachineOperand::MO_GPUMask);
    Op.setGPUMask(GPUMask);
    return Op;
  }
