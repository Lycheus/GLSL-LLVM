...
void SC_VS::access()
{
    ...
    {      
      ...
      {  
          ...
          //=====================================================
	  //fixed by sheng-chih
          if(address<3000)  //read instruction
	  ...
	  //fixed by sheng-chih
           else if(address<5000)  //read Uniform data
	...
	//end fixed
	  ...
	  //else if(address<4000) //read vertex data
           else if(address<41000) //read vertex data
           {   
           	address -= 6000;
           	//address -= 3000;
      //end fixed
           	...
           	Ex_MEM[v_num][v_entry_num][v_word_num]  = *((float*)((void*)&data));
		//printf("Ex_MEM:%f\n",Ex_MEM);
     ...
     //end fixed
                 ...
                 for(i=0;i<4;i++)
                 {
                   Rs_swiz_temp[i]=0; Rt_swiz_temp[i]=0; Rd_swiz_temp[i]=0;
		  ...
		  for(i=0;i<clip_model.vertex_number_out;i++)
		      ...
		      for(j=0;j<4;j++)
			{
		      ...
		      */
		      ...
		      for(j=0;j<4;j++)
			{
			  mp_VOB.write(VOB[0][j]);//CYL:Push Vertex to FIFO
		      ...
		      for(j=0;j<4;j++)
			{
			  mp_VOB.write(VOB[1][j]);//CYL:Push Vertex to FIFO
		      ...
		      for(j=0;j<4;j++)
			{
			  mp_VOB.write(VOB[2][j]);//CYL:Push Vertex to FIFO
		      ...
		      for(j=0;j<4;j++)
			{
			  mp_VOB.write(VOB[3][j]);//CYL:Push Vertex to FIFO
...
//-------------printf for debug------------------------------------------------- 
    ...
    switch(IF.op)
    {
       ...
       case 8 : fprintf(fp_Result,"Op:%d = LDV\n", IF.op)             ; break;
       case 9 : fprintf(fp_Result,"Op:%d = STV\n", IF.op)             ; break;
	 ...
	 //case 14: fprintf(fp_Result,"Op:%d = STV\n", IF.op)            ; break;
       ...
       case 27: fprintf(fp_Result,"Op:%d = BGT\n", IF.op)            ; break;
...
//------------------------------------------------------------------------------	
    ...
    switch(IF.op){
	 ...
	 //end added
	    ...
	    //memcpy(&temp_float,&Rs_swiz_temp[0],32);
	    //fprintf(fp_Result,"temp_float: %f\n",temp_float);
	    fprintf(fp_Result,"Rs_swiz_temp[0]: %lf\n",Rs_swiz_temp[0]);
       ...
       case 7:
	    ...
	    fprintf(fp_Result, "IF.Sw_d_w:%d_%d_%d_%d\n", IF.Sw_d_x, IF.Sw_d_y, IF.Sw_d_z, IF.Sw_d_w);
	    fprintf(fp_Result, "IF.Sw_s_w:%d_%d_%d_%d\n", IF.Sw_s_x, IF.Sw_s_y, IF.Sw_s_z, IF.Sw_s_w);
       ...
       case 10:
            ...
            {
               if(IF.Rd>=32)
                  fprintf(fp_Result,"Rd_VOB_entry[%d] = %lf_%lf_%lf_%lf\n", IF.Rd-32, (VOB[IF.Rd-32][0]), (VOB[IF.Rd-32][1]), (VOB[IF.Rd-32][2]), (VOB[IF.Rd-32][3]));
               else
                  fprintf(fp_Result,"Rd_GPR[%d] = %lf_%lf_%lf_%lf\n", IF.Rd, (GPR[IF.Rd][0]), (GPR[IF.Rd][1]), (GPR[IF.Rd][2]), (GPR[IF.Rd][3]));
       ...
       case 17:
		 ...
		 mulh_temp[i] = Rs_swiz_temp[i] * Rt_swiz_temp[i];               
            }
            // -------------WB----------------------------------
            if(IF.WM_x==1)
       ...
       case 22:
            ...
            for(i=0; i<=IF.S_V; i++)
	      {
                //VS_or_src1.int_format = Rs_swiz_temp[i];
                //VS_or_src2.int_format = Rt_swiz_temp[i];
		//printf("RS.int: %d\nRS.float: %f\n",Rs_swiz_temp[i],Rs_swiz_temp[i]);
		//printf("Rt.int: %d\nRt.float: %f\n",Rt_swiz_temp[i],Rt_swiz_temp[i]);
		memcpy(&VS_or_src1.int_format,&Rs_swiz_temp[i],4);
		memcpy(&VS_or_src1.float_format,&Rs_swiz_temp[i],4);
		//printf("src1.int: %d %f\n",VS_or_src1.int_format,VS_or_src1.int_format);
		//printf("src1.float: %d %f\n",VS_or_src1.float_format,VS_or_src1.float_format);
		memcpy(&VS_or_src2.int_format,&Rt_swiz_temp[i],4);
		memcpy(&VS_or_src2.float_format,&Rt_swiz_temp[i],4);
		//printf("src2.int: %d %f\n",VS_or_src2.int_format,VS_or_src2.int_format);
		//printf("src2.float: %d %f\n",VS_or_src2.float_format,VS_or_src2.float_format);
                do_or();
                or_temp[i] = VS_or_dst.float_format; 
		//or_temp[i] = int(Rs_swiz_temp[i]) | int(Rt_swiz_temp[i]);
		//or_temp[i] = *(int *)&(Rs_swiz_temp[i]) | *(int *)&(Rt_swiz_temp[i]);
	      }
       ...
       case 25:
               ...
               else
		 {
	      ...
	      //end fixed
            ...
            fprintf(fp_Result,"CMEM[480] = %f_%f_%f_%f\n", CMEM[480][0], CMEM[480][1], CMEM[480][2], CMEM[480][3] );                       
            break;
       // BLT
       case 26:
            //
       ...
       case 28:
            ...
            fprintf(fp_Result,"Rs_GPR[%d]:%lf_%lf_%lf_%lf\n", IF.Rs, (Rs_swiz_temp[0]), (Rs_swiz_temp[1]), (Rs_swiz_temp[2]), (Rs_swiz_temp[3]));
            fprintf(fp_Result,"Rt_GPR[%d]:%lf_%lf_%lf_%lf\n", IF.Rt, (Rt_swiz_temp[0]), (Rt_swiz_temp[1]), (Rt_swiz_temp[2]), (Rt_swiz_temp[3]));
            fprintf(fp_Result,"-----------------------------------------------------------------\n");
            //------------ out -----------------
            fprintf(fp_Result,"GPR[%d] = %lf_%lf_%lf_%lf\n", IF.Rd, (GPR[IF.Rd][0]), (GPR[IF.Rd][1]), (GPR[IF.Rd][2]), (GPR[IF.Rd][3]));
            //--------get hex floating point result------------------------------------------------------------
            fprintf(fp_Result,"Rd_GPR[%d] = ", IF.Rd);
            get_fix(int(GPR[IF.Rd][0]));
            fprintf(fp_Result,"_");
