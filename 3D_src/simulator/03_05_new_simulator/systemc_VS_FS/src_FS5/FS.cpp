...
void FS::fragment(struct render_param q)
{
    ...
    GPR[33].range(95,64) = float2bv(t);
    GPR[33].range(127,96) = float2bv(s);   
...
{   
	...
	//ori  process_triangle.u1 = uint2float(SLAVE_MEM[i]) / process_triangle.w1;
	...
	//ori  process_triangle.v1 = uint2float(SLAVE_MEM[i]) / process_triangle.w1;
	...
	i++;
        ...
        process_triangle.uu1 = uint2float(SLAVE_MEM[i]);
        process_triangle.uu2 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.uu3 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
	i++;
        //i+=3;

        process_triangle.vv1 = uint2float(SLAVE_MEM[i]);
        process_triangle.vv2 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.vv3 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
	i++;

        process_triangle.r1 = uint2float(SLAVE_MEM[i]);
        process_triangle.r2 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.r3 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
        i++;
        
        process_triangle.g1 = uint2float(SLAVE_MEM[i]);
        process_triangle.g2 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.g3 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
        i++;
        
        process_triangle.b1 = uint2float(SLAVE_MEM[i]);
        process_triangle.b2 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.b3 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
        i++;

        process_triangle.a1 = uint2float(SLAVE_MEM[i]);
        process_triangle.a2 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.a3 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
	//i-=7;
        i++;
	
        process_triangle.va11 = uint2float(SLAVE_MEM[i]);
        process_triangle.va12 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.va13 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
        i++;

        process_triangle.va21 = uint2float(SLAVE_MEM[i]);
        process_triangle.va22 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.va23 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
        i++;

        process_triangle.va31 = uint2float(SLAVE_MEM[i]);
        process_triangle.va32 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.va33 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
        i++;

        process_triangle.va41 = uint2float(SLAVE_MEM[i]);
        process_triangle.va42 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4)]);
        process_triangle.va43 = uint2float(SLAVE_MEM[i+(VARYING_NUM*4*2)]);
	i++;
...
void DECODE::IR_ctrl(sc_uint<64> IM_data)
{
  ...
  //end added
    ...
    //end added
    //sheng-chang added   
    if(opcode.to_uint() >=24 && opcode.to_uint() <= 29)
      if(BNEQ())
	pc += branch_addr-1; 
...
{    
	...
	//end added
	  ...
	  */
	  ...
	  CMEM_EX[CMEM_offset].range(31,0) = W0; 
	  FS_log_out<<"\nCMEM_offset: "<<CMEM_offset<<endl;
	  FS_log_out<<"CMEM_EX["<<CMEM_offset<<"].range(127,96): "<<uint2float(CMEM_EX[CMEM_offset].range(127,96))<<"\tX0: "<<X0<<endl;
	  FS_log_out<<"CMEM_EX["<<CMEM_offset<<"].range(95,64): "<<uint2float(CMEM_EX[CMEM_offset].range(95,64))<<"\tY0: "<<Y0<<endl;
	  FS_log_out<<"CMEM_EX["<<CMEM_offset<<"].range(63,32): "<<uint2float(CMEM_EX[CMEM_offset].range(63,32))<<"\tZ0: "<<Z0<<endl;
	  FS_log_out<<"CMEM_EX["<<CMEM_offset<<"].range(31,0): "<<uint2float(CMEM_EX[CMEM_offset].range(31,0))<<"\tW0: "<<W0<<endl;
	  //end fixed
        }
...
void GPU_FRAG_IF::behavior(void)
{
	...
	*/
	...
	//fixed by sheng-chih
        ...
        Z = (unsigned int)uint2float(R1.range(63,32));
