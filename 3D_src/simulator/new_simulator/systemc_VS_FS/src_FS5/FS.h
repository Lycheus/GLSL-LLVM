...
SC_MODULE(DECODE)
{
 public:
	  ...
	  sc_fifo_out<bool>			  d_GPR_IF_sig_out;
...
SC_MODULE(EXE)
{
 public:
	  ...
	  sc_fifo_in<bool>		     e_GPR_IF_sig_in;
	  ...
	  sc_fifo_out<bool>			 e_GPR_IF_sig_out;
 ...
 private:
    ...
    sc_bv<32> float2bv(float input);
 	  ...
 	  int RSQ_factor(void);
#else
 	  sc_bv<128> RCP_factor(void);
 	  sc_bv<128> RSQ_factor(void);
#endif
...
SC_MODULE(WB)
{
 public:
	...
	sc_fifo_in<bool>		  b_GPR_IF_sig_in;
...
SC_MODULE(GPU_FRAG_IF)
{
 public:
 	  ...
 	  {
 	      SC_THREAD(behavior);
	      debugbuff.open("./output/debugbuff.txt");
 	  }
 private:
 	  sc_bv<128> *FRAG_IF; 	
 	  void behavior(void);
...
SC_MODULE(FS)
{
 public:
	...
	SC_CTOR(FS):top_FETCH_start(1),top_DECODE_start(1),top_WB_start(1),top_FRAG_finish(1),//CYLin
	  	          ...
	  	          top_w_mask_DX(1),GPR_IF_sig_DX(1),top_RL(1),top_FXL(1),top_sa(1),top_imm_value(1),
	  	          ...
	  	          top_opcode_XB(1),top_dst_XB(1),top_w_mask_XB(1),GPR_IF_sig_XB(1),top_old_DST_XB(1),
	...
	{	   	  	
		...
		I_D->d_GPR_IF_sig_out(GPR_IF_sig_DX);
		I_D->d_RL_out(top_RL);
		...
		I_X->e_GPR_IF_sig_in(GPR_IF_sig_DX);
		I_X->e_RL_in(top_RL);
		...
		I_X->e_GPR_IF_sig_out(GPR_IF_sig_XB);
		...
		I_W->b_GPR_IF_sig_in(GPR_IF_sig_XB);
...
#else
		IM = new sc_uint<64>[IM_LENGTH_EXPAND];//Kaymon
#endif
		GPR = new sc_bv<128>[GPR_LENGTH];
		FRAG_REG = new sc_bv<128>[3];
		CMEM = new sc_bv<128>[CMEM_LENGTH];
		DMEM = new sc_bv<128>[CMEM_LENGTH];
		...
		I_X->DMEM_EX = DMEM;
		I_W->GPR_WB = GPR;
		I_W->FRAG_WB = FRAG_REG;
		//G_F_if->GPR_IF = GPR;
		G_F_if->FRAG_IF = FRAG_REG;
 ...
 private: 	
	...
	sc_uint<64>  *IM;       //[IM]
	sc_bv<128>   *GPR;      //[GPR]
	sc_bv<128>   *FRAG_REG;  //interface register
	sc_bv<128>    *CMEM;     //[CMEM]    
	sc_bv<128>    *DMEM;     //[DMEM] 
};
